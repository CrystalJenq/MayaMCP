...
# 步驟 2: 以 MayaCode 擴充功能預期的 MEL 模式開啟通訊埠
try:
    # 再次檢查以確保通訊埠已關閉
    if not cmds.commandPort(PORT_NAME, query=True):
        print(f"正在以 MEL 模式開啟通訊埠 {PORT_NAME}...")
        cmds.commandPort(name=PORT_NAME, sourceType='mel', echoOutput=True)
        print(f"--- 設定完成！通訊埠 {PORT_NAME} 已在 MEL 模式下成功開啟。Maya 已準備就緒。 ---")
    else:
